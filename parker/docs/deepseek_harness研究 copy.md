...
#### 2.2.9 实操第四课：修改模型能力后，当前对话立即获得图片输入

本次使用中最令人意外的案例不是“模型支持图片”本身，而是：一个手动配置的 OpenAI 兼容模型最初被 DSH 按纯文本模型处理，在 `settings.yaml` 中补充下面一行并保存后，**不用重启 DSH、不用刷新页面、不用重新选择模型，也不用新建对话，当前对话的下一条消息就能粘贴图片并由模型识别**。
...
`input` 是本地 Harness 对代理端点能力的声明，不需要修改或控制代理商服务器。DSH 无法从多数 OpenAI 兼容 `/models` 接口可靠判断输入模态，因此手动录入、目录又没有描述的模型默认回退为纯文本；`input: [text, image]` 表示允许 Harness 把图片内容发送给该模型，端点是否真的接受图片仍由代理服务最终决定。官方用户指南也把这一项放在 `$DSH_HOME/settings.yaml`，而不是代理商后台配置中。
...
它能在当前对话立即生效，不只是因为文件系统有一个 watcher，而是 Settings、LLM Adapter 和会话模型选择共同遵循运行时解析：

1. `settings-file` 使用 `chokidar` 监听用户设置文件，并通过 `awaitWriteFinish` 等待编辑器完成原子替换或分段写入；变化进入串行 refresh 队列，避免多次文件事件并发覆盖。
2. `SettingsProvider` 重新解析文档，以 `llm-pi-ai` 注册的 schema 和额外约束校验配置；只有 resolved value 真正变化时才提交新的不可变快照，并按 namespace 通知 watcher。
3. `llm-pi-ai` 通过 `installSettingsSection(...)` 订阅自己的配置段。它保存的是“读取当前配置的 source thunk”，不是启动时永久捕获的一份对象；模型、端点和能力描述的变化因此能被下一次解析读取。
4. 当前 Session 保存的是所选 `provider + model`，而不是把该模型的全部能力元数据复制并冻结在会话中。图片 Prompt 入场时，Host 会再次调用 `resolveModelInfo(current.provider, current.model)`；更新后的 `inputModalities` 已包含 `image`，所以同一会话的下一次请求被放行。

```text
settings.yaml 保存
        ↓
settings-file 文件监听与串行 refresh
        ↓
SettingsProvider 校验并提交 llm-pi-ai 新快照
        ↓
namespace watcher 通知 PiAiAdapter
        ↓
当前 Session 保持原 provider/model 选择
        ↓
下一次 Prompt 重新解析模型能力
        ↓
图片输入立即生效
...
这个案例适合放在 “Everything is a plugin” 下，但更准确地说，它展示的是 **Cordis 响应式组合在产品配置层的落地**：Settings Provider 是可替换能力，`llm-pi-ai` 是它的消费者，配置变化通过运行时依赖和 watcher 局部传播，而不是把整个 Agent 进程销毁重建。它与传统 CLI 常见的“启动时读取配置 → 构造客户端 → 进程内一直持有”形成鲜明对照。

同时必须保留两个边界：第一，热更新的是 Harness 的能力声明，不能让原本不支持图片的代理端点凭空获得视觉能力；第二，已经写入 Session Log 的图片仍是会话事实，若把模型改回纯文本模型，DSH 会阻止选择或请求，以免后续重放持续向不兼容模型发送图片。

对应源码与官方说明：

- [`packages/settings/settings-file/src/index.ts`](../../packages/settings/settings-file/src/index.ts)：文件监听、写入稳定等待和 refresh 队列。
- [`packages/settings/settings/src/index.ts`](../../packages/settings/settings/src/index.ts)：namespace 注册、resolved snapshot、watcher 与 `settings/updated` 提交。
- [`packages/llm/llm-pi-ai/src/index.ts`](../../packages/llm/llm-pi-ai/src/index.ts)：`installSettingsSection` 和动态 profile source。
- [`packages/llm/llm-pi-ai/src/adapter.ts`](../../packages/llm/llm-pi-ai/src/adapter.ts)：把模型 `input` 暴露为 `inputModalities`，并在请求前校验图片能力。
- [`packages/host/apiproxy/src/api-proxy.ts`](../../packages/host/apiproxy/src/api-proxy.ts)：Prompt 入场和切换模型时的图片能力检查。
- [模型提供方配置指南](../../docs/user/guide/providers.zh.md#图片输入)：手动视觉模型的 `input: [text, image]` 配置方法和边界。
